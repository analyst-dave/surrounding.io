...
const Narratives: Record<string, string[]> = {
    "Work": ["Did you see the Q3 reorg email?", "Sarah resigned, taking the leads with her.", "The D3 logic is finally stable.", "CEO wants a demo of the spatial map."],
    "Family": ["Aunt May's pie is officially toxic.", "Mark's mansion moat is leaking.", "Thanksgiving is in Tahoe this year.", "Mom found the old albums."],
    "Church": ["Pastor's Vegas secret is out.", "Bake sale hit the 10k target.", "Youth group needs a new leader.", "Audit starts on Friday."],
    "Investors": ["Burn rate is $180k. We have 14 months.", "Series A term sheet is signed. 25M post-money.", "Lead VC wants observer rights.", "User retention is up 40%."],
    "DevTeam": ["Elasticity target set to 0.1.", "React flow vs D3?", "Websocket latency is high.", "Merge the hotfix."],
...
async function seed() {
  console.log("🌱 Starting Database Seed (Full D3 Topology)...");
  ...
  // Update main user location (Sacramento base)
  ...
  // 30 Discoverable Nodes (Nearby on map, but not connected)
  for(let i=69; i <= 98; i++) {
    const gender = i % 2 === 0 ? 'men' : 'women';
    const faceId = Math.floor(i / 2) % 100;
    nodes.push({
  ...
  console.log("Setting spatial locations...");
  for (const n of nodes) {
    if (!authIds[n.id]) continue;
    // Spread them out up to ~10-15 miles (0.2 degrees)
  ...
  // Create direct connections for the 68 nodes
  ...
  for (const n of connectedNodes) {
    ...
    if (conn) {
      // Inject history
      const typeStr = n.group === 'singles' ? 'Singles' : ["Work", "Family", "Church", "Investors", "DevTeam"][n.group as number];
      const pool = Narratives[typeStr] || Narratives["Work"];
      const messagesToInsert = [];
      
      let baseTime = new Date(Date.now() - 1000 * 60 * 60 * 24 * 7); // 7 days ago
      
      const numMsgs = Math.floor(Math.random() * 8) + 3; // 3 to 10 messages
      for(let j=0; j < numMsgs; j++) {
        baseTime = new Date(baseTime.getTime() + 1000 * 60 * Math.random() * 60); // Random offset up to 1 hour
  ...
  console.log("Building mutual connections topology...");
  // Mutual Connections among the 5 groups
  ...
  console.log("✅ Database successfully seeded with 98 nodes (68 connected, 30 discoverable)!");
}
